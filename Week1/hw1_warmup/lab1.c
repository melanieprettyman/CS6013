...
{
    //---------------------------------------
    //      BYTE & NIBBLE SORT TESTS
    //----------------------------------------
     //test cases
    unsigned long test_cases[] = {
        0x0000000000000000,  //input with all zeros
        0xFFFFFFFFFFFFFFFF,  //input with all ones
        0x0102030405060708,  //input with reverse-sorted bytes
        0x0807060504030201   //input with sorted bytes

    };

    //expected byte results for the test cases
    unsigned long expectedResultsByte[] = {
        0x0000000000000000,
        0xFFFFFFFFFFFFFFFF,
        0x0807060504030201,
        0x0807060504030201
    ...
    //expected nibble results for the test cases
    unsigned long expectedResultsNibble[] = {
            0x0000000000000000,
            0xFFFFFFFFFFFFFFFF,
            0x8765432100000000,
            0x8765432100000000
    ...
    for (int i = 0; i < numTests; ++i) {
        ...
        //Assert expected values match results
        assert(result == expectedResultsByte[i]);
    }
    //Nibble tests
    for (int i = 0; i < numTests; ++i) {
        unsigned long result = nibble_sort(test_cases[i]);
        //Assert expected values match results
        assert(result == expectedResultsNibble[i]);
    }
    ...
    char expectedName[] = "Davison"; // Change this according to the name used

    //TEST: LIST FUNCTIONALITY
    //assert that the val and link are correct
    ...
    //TEST FREE LIST
    //free_list to deallocate memory
    free_list(head);

    //list should be NULL after freeing)
    assert(head == NULL);

    draw_me();


    //Lindsey Haslam Tests
    //TEST BYTE SORT!
    testByteSort();
    return 0;
}
